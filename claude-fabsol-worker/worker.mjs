...
/** Stop FabKim / Moonshot so FabSol owns the session env exclusively. */
  ...
  await Promise.all(
    ports.map(async (port) => {
      try {
        ...
        log(`sibling session on :${port} stopped for FabSol`);
      } catch {
...
/** Install skills/commands/agents/scripts into ~/.claude so /fabsol works in Claude Desktop + Cursor. */
function ensureClaudeManualInstall() {
  ...
  const pairs = [
    [path.join(MODEL_ROUTER_SRC, 'skills', 'fabsol'), path.join(CLAUDE_HOME, 'skills', 'fabsol')],
  ...
  const fabsolSkill = path.join(CLAUDE_HOME, 'skills', 'fabsol', 'SKILL.md');
  if (fs.existsSync(fabsolSkill)) {
    let text = fs.readFileSync(fabsolSkill, 'utf8');
    text = text
...
FabSol is enabled from the management panel.
`;
    }
    fs.writeFileSync(fabsolSkill, text, 'utf8');
  }

  notes.push(`skills/commands/agents synced into ${CLAUDE_HOME}`);
...
async function prepareFabSolEnvironment() {
  const notes = [];
...
async function startSession() {
  ...
  writeSessionBat();
  await prepareFabSolEnvironment();
  ...
  state.lastError = null;
  saveState();
  log(`FabSol session started pid=${pid}`);
  return getStatusPayload();
}
...
async function getStatusPayload() {
  ...
  const proxy = await probeProxy();
  const claudePath = await findClaudeOnPath();
  const proxyClaude = (proxy.models || []).filter((id) =>
    String(id || '').toLowerCase().startsWith('claude-')
  );
  const proxyWorkers = (proxy.models || []).filter(
    (id) => !String(id || '').toLowerCase().startsWith('claude-') &&
      !String(id || '').toLowerCase().startsWith('kimi-') &&
      !String(id || '').toLowerCase().startsWith('moonshot-')
  );
  ...
  const workerModels =
    proxyWorkers.length > 0
      ? Array.from(new Set([config.workerModel, ...proxyWorkers, ...FALLBACK_WORKERS]))
      : FALLBACK_WORKERS.includes(config.workerModel)
        ? FALLBACK_WORKERS
        : [config.workerModel, ...FALLBACK_WORKERS];
  ...
  return {
    ...
    hasOrchestratorModel:
      Boolean(proxy.hasOrchestrator) ||
      String(config.orchestratorModel || '').toLowerCase().startsWith('claude-'),
...
server.listen(config.listenPort, config.listenHost, () => {
  log(`FabSol worker listening on http://${config.listenHost}:${config.listenPort}`);
});
