...
export function QuotaSection<TState extends QuotaStatusState, TData>({
  ...
  const isClaudeSection = config.type === 'claude';
  const {
    ...
    setAccountEnabled,
    isAccountEnabled,
  } = useClaudeAutostart(isClaudeSection);
    ...
    <Card
      ...
      extra={
        <div className={styles.headerActions}>
          {isClaudeSection && (
            <div
              className={styles.autostartGlobal}
              title={
                autostartAvailable
            ...
            >
              <ToggleSwitch
                ...
                disabled={disabled || !autostartAvailable || autostartLoading || autostartSaving}
                label={t('claude_quota.autostart_global_label')}
        ...
        <>
          <div ref={gridRef} className={config.gridClassName}>
            {pageItems.map((item) => {
              ...
              const accountAutostartOn = isAccountEnabled(item.name);
              const extraActions =
                isClaudeSection && autostartAvailable ? (
                  <div
                    className={styles.autostartAccount}
                    title={t('claude_quota.autostart_account_hint')}
                  >
                    <ToggleSwitch
                      checked={accountAutostartOn}
                      onChange={(value) => void setAccountEnabled(item.name, value)}
                      disabled={
                        disabled ||
                        item.disabled ||
                        !autostartSettings?.globalEnabled ||
                        autostartSaving
                      }
                      label={t('claude_quota.autostart_account_label')}
                      ariaLabel={t('claude_quota.autostart_account_label')}
                    />
                  </div>
                ) : undefined;
