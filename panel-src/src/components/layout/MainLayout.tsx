...
import { useFabSolAndon } from '@/pages/hooks/useFabSolAndon';
import { useFabKimAndon } from '@/pages/hooks/useFabKimAndon';
import { useClaudeAndon } from '@/pages/hooks/useClaudeAndon';
import { useMoonshotAndon } from '@/pages/hooks/useMoonshotAndon';
import {
...
const sidebarIcons: Record<string, ReactNode> = {
  ...
  fabSol: <IconSidebarFabSol size={18} />,
  fabKim: <IconSidebarFabKim size={18} />,
  claude: <IconSidebarClaude size={18} />,
  moonshot: <IconSidebarMoonshot size={18} />,
...
export function MainLayout() {
  ...
  const { tone: fabSolAndonTone } = useFabSolAndon(10);
  const { tone: fabKimAndonTone } = useFabKimAndon(10);
  const { tone: claudeAndonTone } = useClaudeAndon(10);
  const { tone: moonshotAndonTone } = useMoonshotAndon(10);
  ...
  const navGroups: SidebarNavGroup[] = [
    {
      ...
      items: [
        ...
        },
        {
          path: '/fabsol',
          labelKey: 'nav.fabsol',
          metaKey: 'nav_meta.fabsol',
          icon: sidebarIcons.fabSol,
        },
        {
          path: '/fabkim',
          labelKey: 'nav.fabkim',
          metaKey: 'nav_meta.fabkim',
          icon: sidebarIcons.fabKim,
        },
        {
          path: '/claude',
          labelKey: 'nav.claude',
          metaKey: 'nav_meta.claude',
          icon: sidebarIcons.claude,
        },
        {
          path: '/moonshot',
          labelKey: 'nav.moonshot',
          metaKey: 'nav_meta.moonshot',
          icon: sidebarIcons.moonshot,
        },
  ...
  const renderNavLink = (item: SidebarNavLinkItem, className = 'nav-item') => {
    ...
    return (
      <NavLink
        ...
        to={item.path}
        className={({ isActive }) =>
          `${className}${isSessionNav ? ' nav-item-session' : ''}${isActive ? ' active' : ''}`
        }
      ...
      >
        <span className="nav-icon">
          {item.icon}
          {sessionAndon ? (
            <span className={`nav-andon nav-andon-${sessionAndon}`} aria-hidden="true" />
          ) : null}
        </span>
        {showSidebarLabels && (
