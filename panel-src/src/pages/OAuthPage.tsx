...
/** Map OAuth flow provider id → auth-file type key used by /auth-files. */
const oauthProviderToAuthType = (providerId: string): string => {
...
export function OAuthPage() {
  ...
  const [loginTotal, setLoginTotal] = useState<number | null>(null);
  const [loginCountsByType, setLoginCountsByType] = useState<Record<string, number>>({});
  const [loginCountsLoading, setLoginCountsLoading] = useState(false);
  ...
  const refreshLoginCounts = useCallback(async () => {
    setLoginCountsLoading(true);
    try {
      ...
      setLoginTotal(files.length);
      setLoginCountsByType(counts);
    } catch {
      setLoginTotal(null);
      setLoginCountsByType({});
    } finally {
      setLoginCountsLoading(false);
    }
  }, []);

  useEffect(() => {
    void refreshLoginCounts();
  }, [refreshLoginCounts]);
  ...
  const renderOAuthProviderCard = (provider: OAuthProviderCard, featured = false) => {
    ...
    const statusBadgeClassName = [
      ...
      .filter(Boolean)
      .join(' ');
    const providerLoginCount = loginCountsByType[oauthProviderToAuthType(provider.id)] || 0;
      ...
      <Card
        ...
        title={
          <span className={styles.cardTitle}>
            ...
            <span
              className={`${styles.loginCountBadge} ${
                providerLoginCount > 0 ? styles.loginCountBadgeActive : ''
              }`.trim()}
            >
              {t('auth_login.provider_login_count', { count: providerLoginCount })}
            </span>
          </span>
        }
        extra={
  ...
  const loginBreakdown = Object.entries(loginCountsByType)
  ...
  return (
    <div className={styles.container}>
      <div className={styles.pageHeader}>
        ...
        <div className={styles.loginStatusBar} aria-live="polite">
          <div className={styles.loginStatusMain}>
            <span className={styles.loginStatusLabel}>
              {t('auth_login.login_status_label')}
            </span>
            <strong className={styles.loginStatusValue}>
              {loginCountsLoading && loginTotal === null
                ? '...'
                : t('auth_login.login_status_total', { count: loginTotal ?? 0 })}
            </strong>
          </div>
          {loginBreakdown.length > 0 && (
            <div className={styles.loginStatusChips}>
              {loginBreakdown.map(([type, count]) => (
                <span key={type} className={styles.loginStatusChip}>
                  {getTypeLabel(t, type)}: {count}
                </span>
              ))}
            </div>
          )}
          <Button
            ...
            disabled={loginTotal === null && !loginCountsLoading}
          >
            {t('auth_login.view_auth_files')}
          </Button>
        </div>
